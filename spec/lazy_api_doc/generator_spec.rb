  ...
  describe '#query_params' do
    subject do
      LazyApiDoc::Generator.new.send(
        :query_params,
        {'verb' => ['GET']},
        [OpenStruct.new('request' => { 'full_path' => "http://example.com?#{query}"}, 'params' => { 'f' => [1,2,3] }, 'verb' => 'GET')]
      )
    end
    ...
    it {
      is_expected.to eq(
        [
          ...
          {"in"=>"query", "required"=>true, "name"=>"f", "schema"=>{"type"=>"array", "items"=>{"type"=>"integer", "example"=>1}, "example"=>[1, 2, 3]}}
        ]
      )
    }
  end
